...
const Do = () => {
    ...
    const [session, todos, setTodos] = useStore(useShallow((state) => [state.session, state.do_todos, state.set_do_todos]));
    ...
    return (
        <div className='bg-do'>
            <div className='flex px-2 justify-between bg-doOverlay m-2 rounded'>
                    ...
                    <div {...provided.droppableProps} ref={provided.innerRef} className='m-2'>
                        {todos.map((todo, idx) => (
                            <Draggable key={todo.id} draggableId={todo.id.toString()} index={idx}>
                                {(provided) => {
                                    return (
                                        <div className='mb-2 bg-doOverlay p-1 rounded' {...provided.dragHandleProps} {...provided.draggableProps} ref={provided.innerRef}>
                                            {todo.todo}
                                        </div>
                                    )
                                }}
