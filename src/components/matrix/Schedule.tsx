...
const Schedule = () => {
    ...
    const [session, todos, setTodos] = useStore(useShallow((state) => [state.session, state.schedule_todos, state.set_schedule_todos]));
    ...
    return (
        <div className='bg-schedule'>
            <div className='flex px-2 justify-between bg-scheduleOverlay m-2 rounded'>
                ...
                <Dialog open={open} onOpenChange={setOpen}>
                    ...
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>Add the tasks which you need to schedule</DialogDescription>
                        </DialogHeader>
                    ...
                    <div {...provided.droppableProps} ref={provided.innerRef} className='m-2'>
                        {todos.map((todo, idx) => (
                            <Draggable key={todo.id} draggableId={todo.id.toString()} index={idx}>
                                {(provided) => {
                                    return (
                                        <div className='mb-2 rounded p-1 bg-scheduleOverlay' {...provided.dragHandleProps} {...provided.draggableProps} ref={provided.innerRef}>
                                            {todo.todo}
                                        </div>
                                    )
                                }}
